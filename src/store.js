...
export default class Store {
   ...
   * @return {Store} this
   */
  featureCreated(featureId, options = {}) {
   ...
   * @return {Store} this
   */
  featureChanged(featureId, options = {}) {
    this._changedFeatureIds.add(featureId);
  ...
  delete(featureIds, options = {}) {
    ...
    toDenseArray(featureIds).forEach((id) => {
      ...
      if (!options.silent) {
        if (!deletedFeaturesToEmit.includes(this._features[id])) {
  ...
  setSelected(featureIds, options = {}) {
    ...
    this.deselect(this._selectedFeatureIds.values().filter(id => !featureIds.includes(id)), { silent: options.silent });
  ...
  getSelectedCoordinates() {
    const selected = this._selectedCoordinates.map(({feature_id, coord_path}) => {
...
function refreshSelectedCoordinates(store, options = {}) {
  const newSelectedCoordinates = store._selectedCoordinates.filter(({feature_id}) => store._selectedFeatureIds.has(feature_id));
