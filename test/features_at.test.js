...
import {featuresAtClick, featuresAtTouch} from '../src/lib/features_at.ts';
...
/**
 * Mock context with a simplified maplibre-gl-js map (including some source/style/layer interactions)
 */
...
test('featuresAt with click bounding box', () => {
  ...
  const result = featuresAtClick(null, [[10, 10], [20, 20]], mockContext);
...
test('featuresAt with touch bounding box', () => {
  const mockContext = createMockContext();
  const result = featuresAtTouch(null, [[10, 10], [20, 20]], mockContext);
...
test('featuresAt should not include missing style layers', () => {
  ...
  let result = featuresAtTouch(null, [[10, 10], [20, 20]], mockContext);
  ...
  result = featuresAtTouch(null, [[10, 10], [20, 20]], mockContext);
  assert.deepEqual(result, [{
    type: 'Feature',
    properties: {
      meta: 'vertex',
