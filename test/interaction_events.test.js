...
test('ensure user interactions fire right events', async (t) => {
  ...
  const pointButton = controlGroup.getElementsByClassName('maplibregl-draw_point')[0];
  const lineButton = controlGroup.getElementsByClassName('maplibregl-draw_line')[0];
  const trashButton = controlGroup.getElementsByClassName('maplibregl-draw_trash')[0];
  const polygonButton = controlGroup.getElementsByClassName('maplibregl-draw_polygon')[0];
...
test('ensure API fire right events', async (t) => {
  ...
  const Draw = MaplibreDraw({ suppressAPIEvents: false });
